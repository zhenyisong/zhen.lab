---
title     : " R markdown to explain the protocol in the paper "
author    : " Yisong Zhen "
date      : " 18 May 2020 "
...
In this protocol, I will step by step describe the analysis procedure.
The first step is to load the required libraries into the global enviroment.

```{r all_required_library, echo=TRUE}
...
In the following CONSTANT definition block, I will define the 
deposition layout of the raw data and annotation files. If you 
would like to replicate the analysis, the absolate path should be
changed accordingly to adapt to your own working directory.


```{r constant_definition, echo=FALSE}
#---
...
# the rsubread index files were generated seperatedly
...
# data output path and you should change it accordingly.
...
After the CONSTANT definition, I will define the data layout to save
the raw data and analysis results. This will facilitate the  future replication
work. the raw.path is the absolute data path.

```{r layout_function, echo=FALSE}
#---
...
```{r defined_functions, echo=FALSE}
...
This is an inner function which is used to get the expression value of dataset
GSE64403.
@params the featureCount of the GSE64403
@ouput  the Excel file of this dataset, including the expression value.
```{r quality_control}
"
vAd  : Adult ventricular myocardium
vD7S : 7 days post-sham surgery ventricular myocardium
...
This function is defined to export the atrium and ventricle gene set. Chamber gene set.seed
is the collate of genes which are enriched in the atrium or ventricle respectively.
@params the affy microarray expression data
@ouput  the excel file which includes two sheets, one is atrium gene set and the other is
        ventricle data
...
Now, we begin to analyze the raw data using previously defined functions.

```{r using_rsubread_align, echo=TRUE, eval=FALSE}
"
...
```{r using_feature_count, echo=TRUE, eval=FALSE}
...
```{r get_the_microarray, echo=TRUE, eval=FALSE}
...
```{r load_preporcessed_data, message=FALSE}

setwd('data/rdata')
load('atriumization.Rdata')
...
```{r figures, cache=TRUE, message=FALSE}
...
```{r figure.caption.1,fig.cap=paste("Figure 1. Curation of atrium specific genes from the benchmark set GSE1479.")}
(figure.1)
```

```{r figure.caption.2, fig.cap=paste("Figure 2. Using atrium gene set in myocardial infarction model and the GSEA result")}
(figure.2)
```

```{r figure.caption.3,fig.cap=paste("Figure  3. Atrium gene expression pattern were replicated in an independent dataset GSE64403. ")}
(figure.3)
```

```{r figure.caption.4,fig.cap=paste("Figure 4. Multiple comparison of Aldh1a2 expression between different groups in dataset GSE64403")}
(figure.4)
```

```{r save_figure_result, echo=TRUE, eval=FALSE}
"
save the figures!
"
